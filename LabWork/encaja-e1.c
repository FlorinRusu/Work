...
int lee_ppm(char nombre[], Imagen *ima)
{
  ...
  while (fscanf(F, " %1[#]", almohadilla) == 1)
    fscanf(F, "%*[^\n]");
...
int main(int argc, char *argv[])
{
  ...
  int nthread;
  if (lee_ppm(entrada, &ima))
    return 2;

  t1 = omp_get_wtime();
  encaja(&ima);
  t2 = omp_get_wtime();
  printf("Time: %f", t2 - t1);  //Calculate time

#pragma omp parallel
  {
    nthread = omp_get_num_threads(); // Display the number of threads!!!
  }
  printf("Number of threads = %d\n", nthread);
  if (escribir)
    if (escribe_ppm(salida, &ima))
      return 3;
